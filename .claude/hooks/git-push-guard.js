#!/usr/bin/env node
/**
 * Git Push Guard — PreToolUse on Bash — policy-only adapter.
 * Envelope: ./_claude-bash-guard.js. Sanitizer: harness-core.
 * Behavior byte-identical to original (G-OC01 part1 S2, pure refactor).
 *
 * Blocks git push (force = extra warning). Bypass marker:
 * .claude/git-push-approved.marker  { "expiresAt": <ms>, "uses": <n> }
 */
const fs = require("fs");
...
H.start((command) => {
	...
	if (checkAndConsumeApproval()) {
		...
		return null; // Normal push with valid marker — allow
	}
