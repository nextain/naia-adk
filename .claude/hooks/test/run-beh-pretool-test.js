...
// valid handshake + backgrounded → block.
{
	...
	assert(blocked(out) && /미감독 백그라운드/.test(out), "2. backgrounded Bash → block", out.slice(0, 160));
	fs.rmSync(cwd, { recursive: true, force: true });
}
...
{
	const cwd = setup({ withHandshake: false });
	const out = fire("beh-pretool.js", { cwd, session_id: SID, tool_name: "Bash", tool_input: { command: "bash .claude/hooks/beh-launch.sh ." } });
...
function launchFixture(registerAll = true) {
	...
	const hooks = registerAll ? "beh-tick.js beh-record.js beh-stop.js beh-pretool.js" : "beh-tick.js beh-record.js";
	fs.writeFileSync(path.join(cwd, ".claude", "settings.json"), JSON.stringify({ note: hooks }));
	return cwd;
}
// success: all hooks registered + cores load → handshake written.
{
	const cwd = launchFixture(true);
	const r = cp.spawnSync("bash", [path.join(HOOKS, "beh-launch.sh"), cwd], { encoding: "utf8" });
