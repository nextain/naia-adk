...
export function validateBundleSemantics(bundle) {
  ...
  for (const comparison of bundle.comparisons) {
    ...
    const modelOnly=comparison.comparison_scope === "model_only";
    const compositeRoute=comparison.comparison_scope === "model_provider_adapter";
    if (modelOnly && (!comparison.fixed_control_keys.includes("price_snapshot_digest") || reference.price_snapshot.digest !== candidate.price_snapshot.digest)) errors.push(`${comparison.id}: model-only price snapshot mismatch`);
    if (compositeRoute && comparison.fixed_control_keys.includes("price_snapshot_digest")) errors.push(`${comparison.id}: composite route comparison falsely declares price snapshot fixed`);
    if (compositeRoute && (reference.price_snapshot.currency !== candidate.price_snapshot.currency || reference.price_snapshot.normalization_method !== candidate.price_snapshot.normalization_method)) errors.push(`${comparison.id}: composite route price normalization mismatch`);
    ...
    if (comparison.layer === "A_model_capability") {
      const commonControls=reference.product_revision === candidate.product_revision && reference.agent_runtime_revision === candidate.agent_runtime_revision && reference.governance_harness_revision === candidate.governance_harness_revision && reference.tool_adapter_revision === candidate.tool_adapter_revision;
      const validModelOnly=modelOnly && comparison.changed_variable === "model_configuration" && !sameModel && reference.model.provider === candidate.model.provider && reference.adapter_revision === candidate.adapter_revision;
      const routeIdentityChanged=!sameModel || reference.adapter_revision !== candidate.adapter_revision;
      const validComposite=compositeRoute && comparison.changed_variable === "model_provider_adapter" && routeIdentityChanged && comparison.claim_scope !== "marginal_model_inference";
      if (!commonControls || (!validModelOnly && !validComposite)) errors.push(`${comparison.id}: model-layer comparison scope or controls invalid`);
    } else if (comparison.layer === "B_agent_runtime") {
      if (!modelOnly || reference.adapter_revision !== candidate.adapter_revision || comparison.changed_variable !== "agent_runtime_revision" || !sameModel || reference.agent_runtime_revision === candidate.agent_runtime_revision || reference.product_revision !== reference.agent_runtime_revision || candidate.product_revision !== candidate.agent_runtime_revision || reference.governance_harness_revision !== candidate.governance_harness_revision || reference.tool_adapter_revision !== candidate.tool_adapter_revision) errors.push(`${comparison.id}: runtime-layer causal controls invalid`);
    } else {
