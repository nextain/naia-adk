...
export function selectDevelopmentBindingFromCatalog(catalog,{profileId="balanced",role,boundedScope=false,exactValidator=false,risk="medium",producerBinding=null,producerSessionId=null,reviewerSessionId=null,producerExecutionId=null,reviewerExecutionId=null}={}){
  ...
  if(role==="orchestrator"||role==="integrator")binding=profile.assignments[role];
		...
		binding=reviewGuard.prefer_different_binding_from_producer
			? profile.assignments.reviewer_pool.find(item=>item!==producerBinding)??profile.assignments.reviewer_pool[0]
			: profile.assignments.reviewer_pool[0];
...
async function main(argv){
  ...
  if(command==="show")return resolveDevelopmentProfile(flag("--profile")||"balanced");
	if(command==="select")return selectDevelopmentBinding({profileId:flag("--profile")||"balanced",role:flag("--role"),risk:flag("--risk")||"medium",boundedScope:rest.includes("--bounded-scope"),exactValidator:rest.includes("--exact-validator"),producerBinding:flag("--producer-binding"),producerSessionId:flag("--producer-session-id"),reviewerSessionId:flag("--reviewer-session-id"),producerExecutionId:flag("--producer-execution-id"),reviewerExecutionId:flag("--reviewer-execution-id")});
