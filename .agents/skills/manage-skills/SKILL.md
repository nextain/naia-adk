---
name: manage-skills
description: 세션 변경사항을 분석해 verify-* 스킬 드리프트를 탐지하고 자동 생성/업데이트합니다. issue-driven-development Sync 단계, 새 패턴/규칙 도입 후, PR 전 검증 스킬 커버리지 확인 시 반드시 사용. /manage-skills로 호출.
disable-model-invocation: true
argument-hint: "[선택사항: 특정 스킬 이름 또는 집중할 영역]"
---

> **[워크스페이스 규칙]** 이 스킬이 `CLAUDE.md`를 Edit/Write할 때, `sync-entry-points.js` 훅이 자동으로 `AGENTS.md`와 `GEMINI.md`에 동기화합니다. AGENTS.md/GEMINI.md를 수동으로 업데이트할 필요가 없습니다.
...
- **피처 개발 Sync 단계마다** — `issue-driven-development.yaml` 워크플로우에 의해 자동 호출됨 (조건 없이 항상 실행)
...
| `verify-request-contract` | 원요청 source→증거·권한·리뷰·세션 바인딩·양 클라이언트 동등성 | `.agents/hooks/core/{request-contract*,beh-*,harness-core.js,harness-session-inject.test.js}`, `.agents/context/{request-contract.json,harness.yaml,project-index.yaml,skills-index.yaml,complexity-waivers.json}`, `.agents/requirements/{RCI-*.yaml,_index.yaml,sources/**}`, `.claude/{beh-*,hooks/{request-contract.js,session-inject.js},hooks/beh-*,hooks/e2e/*.sh,hooks/test/**,settings.json,skills/verify-request-contract}`, `.codex/**`, `scripts/{*request-contract*,run-native-harness-tests.cjs}`, `packages/artifacts-spec/**`, `.agents/skills/{manage-skills,review-pass,verify-implementation,verify-request-contract}/**`, `.users/{context/{harness,request-contract}.md,skills/verify-request-contract/**}`, `{AGENTS,CLAUDE,GEMINI}.md`, `package.json`, `.gitignore`, `.gitattributes` |
...
| `verify-product-preservation` | 기존 제품 표면·원요청 권한·기준 버전·검토/배포 상태 | `.agents/workflows/issue-driven-development.yaml`, `.agents/requirements/{_template.yaml,**}`, `.agents/skills/{review-pass,verify-implementation,manage-skills,verify-product-preservation,manage-discord-sessions}/**`, `.agents/hooks/core/preservation-*.js`, `.agents/context/{harness.yaml,skills-index.yaml}`, `.users/{context/{harness,request-contract}.md,skills-list.md,skills/{verify-product-preservation,manage-discord-sessions}/**}`, `.claude/skills/verify-*`, `scripts/preservation-*.cjs`, `deploy/preservation-attestor/**`, `{AGENTS,CLAUDE,GEMINI}.md`, `naia-settings/{README.md,messenger-sessions/config.example.json}`, `docs/design/discord-*.md`, project-adapter-declared surfaces/tests/vendor/CI |
