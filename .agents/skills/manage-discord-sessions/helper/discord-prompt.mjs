...
// The notice is emitted as its own prompt part before the `User request:`
// part, never spliced into an already-assembled string. Searching for the
// marker meant a user who pasted a previous prompt containing a
// `User request:` line moved the insertion point into their own text, which
// also broke the suffix the conversation loader matches on.
const MUTATION_WINDOW_CLOSED_NOTICE = "Mutation window: the configured mutation window is currently closed. This attempt is temporarily read-only; an explicit retry after the window opens re-evaluates the original request and current authority.";
