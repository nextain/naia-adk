...
export function renderDiscordHistory(messages, { botUserId, allowedUserIds, messageLimit = DEFAULT_MESSAGE_LIMIT, characterLimit = DEFAULT_CHARACTER_LIMIT } = {}) {
	snowflake(botUserId, "botUserId");
	if (!Array.isArray(messages)) throw new Error("Discord history must be an array");
...
export async function fetchDiscordHistory({ token, channelId, beforeMessageId, botUserId, allowedUserIds, fetchImpl = fetch, messageLimit = DEFAULT_MESSAGE_LIMIT, characterLimit = DEFAULT_CHARACTER_LIMIT, signal } = {}) {
	...
	try {
		...
		const history = renderDiscordHistory(messages, { botUserId, allowedUserIds, messageLimit, characterLimit });
...
export function promptWithDiscordHistory(prompt, history) {
