<!-- AUTO-GENERATED from .agents/context/agents-rules-detail.json — DO NOT EDIT BY HAND.
     SoT = json. Edit the JSON; this file is regenerated by the hook. -->

# agents-rules-detail (mirror)

> Machine SoT: .agents/context/agents-rules-detail.json; this Markdown file is generated.
- **_about**: Full text for sections summarized one line each in .agents/context/agents-rules.json (detail.sections). Same key paths; the logical rule set is the deep merge of both files.
...
- **trigger**: .agents/context/agents-rules.json or .agents/context/agents-rules-detail.json changed
...
- **trigger**: AGENTS.md, CLAUDE.md, or GEMINI.md changed
...
- Treat AGENTS.md as the canonical source. Keep CLAUDE.md and GEMINI.md as the one-line @AGENTS.md import (never copies) and AGENTS.md under 12,000 bytes; a tool-specific mirror must never overwrite the canonical contract
- Validate with node .claude/hooks/sync-entry-points.js --check
