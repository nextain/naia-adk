...
`L1`/`L2`/`L3`와 "three-layer"는 이 저장소에서 서로 다른 세 축으로 쓰인다. 축을 밝히지 않은
`L2`는 추측을 부르고, 추측이 계약이 되면 드리프트다. 2026-08-15에 축을 구분하지 않아 유저 접점
게이트웨이(오케스트레이션 L3)를 "1레이어 제품"으로 오기했고, 그 정의 위에서 2주간 잘못된 층을
측정한 사고가 있었다.

| 축 | 표기 | 뜻 | 출처 |
|---|---|---|---|
| 오케스트레이션 층위 | L1 / L2 / L3 | 누가 누구를 부르고 감시하는가. L3 비서 → L2 이슈 리더 → L1 워커 | `development-model-routing.yaml` |
| 조사 깊이 (IDD scope) | L1 / L2 / L3 | 어디까지 들여다볼 것인가 | `gstack-hooks.md` |
| 벤치마크 인과층 | three-layer | 무엇을 원인으로 분리해 측정하는가. 모델 능력 / naia-agent 런타임 / naia-adk 거버넌스 | `BMC-001` |

오케스트레이션 층위의 감시 방향은 L1←L2, L2←L3, L3←사람 소유자다. 자기 감시는 성립하지 않는다.
문서·이슈·커밋에서 축 없이 `L2`처럼 쓰지 않고, 처음 등장할 때 축을 밝힌다. 축이 불분명한 표기를
만난 AI는 추측하지 말고 되묻는다.
...
프로젝트 고유 도메인 용어(예: naia-corp 의 포털·홈피·lynx)는 각 프로젝트의
