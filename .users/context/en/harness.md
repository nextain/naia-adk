...
- Shared core: `.agents/hooks/core/request-contract.js`, `request-contract-adapter.js`, `request-contract-review-runner.js`, `preservation-contract.js`
...
- The general completion floor is two externally signed Clean reviews bound to the original prompt hash chain and all eight entities/seven edges. Preservation release separately requires four planning roles and four new integration roles with role-specific evidence views.
- The preservation core validates product surfaces, baseline/current paths, capability probes, and imported-source provenance declared by a project-specific adapter. A UI route is only one possible surface kind; APIs, CLIs, library exports, data schemas, background jobs, and operational paths use the same contract.
...
- A terminated directive and its tombstone remain canonically immutable. Isolated reviewer stdout exports only verdict, fixed finding codes, and opaque relationship projections. The general two Clean receipts require distinct contexts/processes. Preservation role separation is a stronger, separate condition: four records produced by relabeling one shared full bundle do not prove planning×4 and integration×4.
...
- Installation and enforcement are separate. Until three public keys, credentials, executable allowlists, and project readiness are provisioned, do not use `REQUEST_CONTRACT=on` or repository enable; keep delivery `REVIEW_ONLY`. `preservation.required` does not override opt-in, and shared projects are not governed until explicitly switched to `enforced`.
- Preservation evidence is also **REQUIRED/PENDING**. The adapter must derive the exact baseline surface set, and an allowlisted trusted runner must execute the real entry and emit a signed probe receipt. Agent-authored JSON is forbidden; vendor evidence requires an origin repository+commit/tree digest attestation.
- `REVIEW_ONLY` checkpoints are local-commit-only. Remote review push remains forbidden until an exact signed checkpoint operation exists. Because no external-effect classifier is implemented yet, the preservation runtime currently blocks every shell/publication event with `external_effect_gate_pending`; built-in release regexes remain diagnostics only.
...
- `USR-001..007` predate the immutable source ledger, and their native messages were not recoverable from local session archives. The validator blocks on that explicit gap rather than reconstructing user provenance from AI-authored requirement prose.
...
- Binding method: Each stage records a **list of receipt IDs, not free-form strings**. Receipts must actually exist, have a Clean finding-0 determination from **different** reviewers meeting the stage minimum, preserve and re-hash each verbatim transcript, list only requirements actually marked COVERED, enumerate every required path under `Files Read`, and match the reviewed working tree's `scope_digest`.
- The digest is derived from current working-tree bytes for tracked and non-ignored untracked files, requirements, and source-ledger artifacts. Only the `reviews:` line and receipt store are excluded so recording a determination does not invalidate it.
...
- The reviewed set includes every current staged, unstaged, and non-ignored untracked change, feature-named paths, every code/test trace, source-ledger artifacts, and a short explicit list. A receipt is rejected if its reviewer omits any required path from `Files Read`. Unchanged dependencies still require a trace or explicit declaration.
- `Files Read` is a reviewer attestation, not mechanical proof that a person or model cognitively inspected each byte. Each new receipt also binds an issuer-derived path, object type, size, and SHA-256 manifest for the exact working-tree objects made available for review; symlinks bind both their link payload and resolved in-repository target bytes, while external targets are rejected. Reviewer isolation and runtime signatures can make the attestation accountable but cannot prove cognition; an ordinary Git receipt remains forgeable by a writer.
...
- Check the general consecutive two-Clean floor
- For preservation, separately verify planning×4 and integration×4 new role executions, role-specific evidence withholding, and stage binding
...
- Require the general two externally signed Clean reviews bound to a private review bundle and separated from the authoring session
- For preservation release, additionally require four planning roles and four new integration roles with distinct context/process/execution, stage binding, and role-specific evidence views
- Reject probe JSON without a trusted adapter-runner receipt, unattested vendor origin, incomplete baseline exact-set, and any remote REVIEW_ONLY publication
