...
Luna implementation is permitted only when the scope is limited and a precise verifier exists. Without a verifier, fall back to Terra; when the scope is not limited or the risk is high, fall back to Sol. Independent reviews must use author and session/execution IDs that differ, and should use a different model binding where possible. Initial availability of `balanced` means that it can be used; it does not mean that cost savings for the full development composition have been demonstrated.
