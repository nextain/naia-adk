...
설정된 검토자가 CLI·인증·가용성 문제로 실행되지 않으면 기본 동작은
fail-closed입니다. 명시적인 `--require-review false` 선택에서만 `NOT_RUN`을
기록하고 일반 로컬 작업을 이어갈 수 있으며, 그 결과는 검토·출판 게이트의
근거가 될 수 없습니다.
...
4. 검토자 합의는 가설일 뿐이므로 조정자가 원문·요구사항·코드·테스트의 1차 근거를 독립 검증합니다.
5. 근거 상태가 `ACCEPTED`인 결함만 자동 수정합니다. `REJECTED`는 수정 없이 기록하고, `UNRESOLVED`는 Clean을 차단합니다.
6. 연속 Clean 기준과 원요청·보존 계약·복잡도 차단 조건을 모두 충족할 때까지 반복합니다.
...
여러 검토자의 합의나 중재자의 판정만으로 수정 권한이나 Clean 판정을 만들 수 없습니다.
