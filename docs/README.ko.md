...
naia-adk                      ← Base 프레임워크 (공개, Apache 2.0)
  └── nextain-adk             ← 포크: 넥스테인 기업 데이터 + 비즈니스 서브모듈
        └── alpha-adk         ← 포크: 개인 데이터 + 프로젝트 서브모듈
```

누구나 자신만의 체인을 시작할 수 있습니다 — `naia-adk`를 포크하고 조직의 데이터를 추가한 뒤, 각 멤버가 다시 포크하여 개인 워크스페이스를 구성합니다.
